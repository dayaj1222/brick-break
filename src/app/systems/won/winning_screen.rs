...
impl Plugin for GameWon {
    fn build(&self, app: &mut App) {
        app.add_systems(OnEnter(GameStates::Won), setup_game_won)
            .add_systems(
                Update,
                handle_gamewon_buttons.run_if(in_state(GameStates::Won)),
            )
            .add_systems(OnExit(GameStates::Won), cleanup);
    }
}
...
fn setup_game_won(mut commands: Commands, score: Res<Score>) {
        ...
        .spawn((
            ...
            children![
                (
                    ...
                    TextColor(Color::srgb(0.25, 0.95, 0.65)),
                ),
                (
